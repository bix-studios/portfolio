    ...
    <section class="section-card" id="${section.id || ""}">
      <div class="about-grid">
        ...
        <div class="section-card status-card">
          <p class="section-sub">Studio Status</p>
          <div class="status-main">
            <span class="status-dot" id="statusDot"></span>
            <span class="status-label" id="statusLabel">Loading</span>
          </div>
          <div class="status-row">
            <span class="status-title">Studio Time</span>
            <span class="status-value" id="nepalTime">--:--</span>
          </div>
          <div class="status-row">
            <span class="status-title">Time Window</span>
            <span class="status-value" id="statusWindow">--</span>
          </div>
          <div class="status-row">
            <span class="status-title">Current Focus</span>
            <span class="status-value" id="statusClient">--</span>
          </div>
        </div>
      </div>
    </section>
...
function renderPage(content) {
  ...
  if (statusDot && statusLabel && nepalTime && statusWindow && statusClient) {
    const ownerZone = content.site.timezoneIana || "Asia/Kathmandu";
    const ranges = [
      {
        label: "Focus Mode",
        dot: "blue",
        ...
        client: "Deep Study & Technical Training",
      },
      {
        label: "Online",
        dot: "green",
        ...
        client: "Development & Client Support",
      },
      {
        label: "Away",
        dot: "yellow",
    ...
    const updateStatus = () => {
      ...
      nepalTime.textContent = timeLabel;
      ...
      statusLabel.textContent = `${active.emoji} ${active.label}`;
      statusWindow.textContent = `${active.start} – ${active.end}`;
      statusClient.textContent = active.client;
    };

    updateStatus();
    if (window.__statusInterval) clearInterval(window.__statusInterval);
    window.__statusInterval = setInterval(updateStatus, 60000);
  }
}
